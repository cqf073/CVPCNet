...
class PrototypeFusion(nn.Module):
    def __init__(self, n_proto=2):
        super().__init__()
        self.weights = nn.Parameter(torch.ones(n_proto))

    def forward(self, prototypes_list):
        x = torch.cat((prototypes_list[0], prototypes_list[1]), dim=0)
        w = torch.softmax(self.weights, dim=0)  # (2,)
        merged = (x * w.unsqueeze(1)).sum(dim=0, keepdim=True)
        return merged


class MLPCompress(nn.Module):
    def __init__(self, in_proto=51, feat_dim=512):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Linear(in_proto, 1),
            nn.ReLU()
        )

    def forward(self, x):  # x: (102, 512)
        half = x.size(0) // 2
        out_list = []
        x1, x2 = x[:half], x[half:]
        for part in [x1, x2]:
            part = part.permute(1, 0)
            compressed = self.fc(part)
            compressed = compressed.permute(1, 0).unsqueeze(dim=0).unsqueeze(-1).unsqueeze(-1)  # (1, 512)
            out_list.append(compressed)
        return out_list
...
class FeatureExpandConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(FeatureExpandConv, self).__init__()
        self.conv = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=65, stride=2, padding=1, output_padding=1)
    def forward(self, x):
        return self.conv(x)
