# CVPCNet
This repo contains the code for our paper "From Stronger to More Reliable: Cross-Granularity Vision-Language Prototype Calibration for Few-Shot Medical Image Segmentation''
![](AssertOfREADME/CVPCNet.png)
...
Download:  
1. **Abdominal MRI/CT**  [Combined Healthy Abdominal Organ Segmentation dataset](https://chaos.grand-challenge.org/)  
2. **Cardiac MRI** [Multi-sequence Cardiac MRI Segmentation dataset (bSSFP fold)](https://zmiclab.github.io/zxh/0/mscmrseg19) 
3. **Image description dataset** [text descriptions for training images]()
...
**Image description dataset**  access to the dataset can be obtained by contacting the author via email: [email address]. Usage of the dataset requires citation of the relevant paper.

### Training  
1. Download pre-trained ResNet-101 weights [vanilla version](https://download.pytorch.org/models/resnet101-63fe2227.pth) or [deeplabv3 version](https://download.pytorch.org/models/deeplabv3_resnet101_coco-586e9e4e.pth) and [longclip-B](https://huggingface.co/BeichenZhang/LongCLIP-B/tree/main) put your checkpoints folder, then replace the absolute path in the code `./models/encoder.py`.  
