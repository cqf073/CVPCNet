...
def main(_run, _config, _log):
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0,'b_loss': 0 ,'loss_PCMR':0 ,"aux_loss" :0 ,'ssp_loss':0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            text_tokens = sample["text_tokens"].squeeze(0).float().cuda()
            ...
            query_pred, align_loss, query_pred_ , b_loss , loss_PCMR ,loss_aux, ssp_loss ,fake_qry_segs= model(support_images, support_fg_mask, query_images ,text_tokens ,train=True)
            ...
            query_loss_ = criterion(torch.log(torch.clamp(query_pred_, torch.finfo(torch.float32).eps, #Loss init loss
                                                         1 - torch.finfo(torch.float32).eps)), query_labels)
            fake_qry_loss = criterion(torch.log(torch.clamp(fake_qry_segs, torch.finfo(torch.float32).eps,
                                                                 1 - torch.finfo(torch.float32).eps)),query_labels.long())
            
            b_loss += criterion_b(query_pred, query_labels)
            loss_PCMR = loss_PCMR +fake_qry_loss
            aux_loss = loss_aux
            ssp_loss = ssp_loss
            loss = query_loss + align_loss + b_loss + loss_PCMR + aux_loss + ssp_loss
            ...
            loss_PCMR = loss_PCMR.detach().data.cpu().numpy()
            ...
            _run.log_scalar('loss_PCMR', loss_PCMR)
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                loss_PCMR = log_loss['loss_PCMR'] / _config['print_interval']
                loss_aux = log_loss['aux_loss'] / _config['print_interval']
                ...
                log_loss['b_loss'] = 0
                log_loss['loss_PCMR'] = 0
                log_loss['aux_loss'] = 0
                log_loss['ssp_loss'] = 0

                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss},'
                          f' align_loss: {align_loss}, b_loss: {b_loss},loss_PCMR:{loss_PCMR}'
                          f' loss_aux: {loss_aux}, ssp_loss :{ssp_loss}')
