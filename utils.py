...
import os
import logging
from models.clip import longclip
...
def compute_dynamic_thresholds(p1, p2, alpha_quantile=0.7, beta_quantile=0.9):
    with torch.no_grad():
        p1_flat = p1.view(-1)
        p2_flat = p2.view(-1)
        overlap_conf = torch.min(p1_flat, p2_flat)
        alpha = torch.quantile(overlap_conf, alpha_quantile).item()
        diff = (p1_flat - p2_flat).abs()
        beta = torch.quantile(diff, beta_quantile).item()

    return alpha, beta

def split_long_text_tokenize(text, max_length=248):
    words = text.split()
    chunks = []
    current_chunk = []
    ...
    for word in words:
        ...
        try:
            test_tokens = longclip.tokenize(test_text, truncate=False)
            if test_tokens.shape[1] <= max_length:
                current_chunk.append(word)
            else:
                if current_chunk:
                    chunk_text = ' '.join(current_chunk)
                    chunks.append(longclip.tokenize(chunk_text, truncate=False))
                current_chunk = [word]
        except RuntimeError:
            if current_chunk:
                chunk_text = ' '.join(current_chunk)
    ...
    return chunks
def process_long_text(text, max_length=248):
    try:
        tokens = longclip.tokenize(text.strip(), truncate=False)
        return [tokens]
    except RuntimeError as e:
        if "too long for context" in str(e):
            return split_long_text_tokenize(text.strip())
        else:
            raise e